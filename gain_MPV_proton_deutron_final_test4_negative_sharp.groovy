...
//     -ptmin 0.20 -ptmax 0.45 -ctrlLo 80 -ctrlHi 480  /path/to/*.hipo
//
// Notes:
...
// - When -submode raw, α falls back to the control‑window robust estimator (LSQ/ratio)
//   and SUB_raw keeps negatives (no flooring). Both modes set per‑bin errors
//   σ_b = sqrt(max(1, S + α^2·B)).
// - All other analysis/plots remain identical to previous versions.
...
final class WireKey{ final int s,Lraw,c; WireKey(int s,int Lraw,int c){ this.s=s; this.Lraw=Lraw; this.c=c }
...
KFRec bestKF_Heavy(DataEvent ev){
  ...
  for(int i=0;i<k.rows();i++){
    ...
    if(score>best){ best=score; out= new KFRec(idx:i, nhits:nh, chi2:chi2, px:k.getFloat("px",i), py:k.getFloat("py",i), trackid:k.getInt("trackid",i))
...
class FitResultGlobal{ boolean ok=false; double amp,mpv,sigma,mpvErr,chi2=Double.NaN; int ndf=0; F1D func=null }
FitResultGlobal fitGlobalLandau(H1F h,double xmin,double xmax){
  ...
  // manual chi2/ndf (use bin errors if present)
  def ax=h.getAxis(); int nBins=ax.getNBins(); int used=0; double chisq=0
  for(int b=0;b<nBins;b++){
    double x=ax.getBinCenter(b); if(x<xmin||x>xmax) continue
    double y=h.getBinContent(b); double err=h.getBinError(b); if(err<=0) err=Math.sqrt(Math.max(1.0,Math.abs(y)))
    if(err<=0) continue
  ...
  f.setLineColor(3); f.setLineWidth(3)
  out.func=f; out.ok=(out.mpv>0 && !Double.isNaN(out.mpv)); return out
}

class FitResult1D{ boolean ok=false; double amp,mpv,sigma,ampErr,mpvErr,sigErr,chi2=Double.NaN; int ndf=0; F1D func=null; }
...
final class PairH{
  ...
  PairH(String tag,String titleBase,int nb,double lo,double hi){
    ...
    sig.setLineColor(4); bkg.setLineColor(2); sub.setLineColor(4)
  }
}
...
@Field H2F H2_PRE_ALL  = new H2F("h2_pt_adc_pre_all", "p_{T} vs ADC (SUB events, RAW/NNLS);p_{T} (GeV);ADC", PT2_NBINS, PT2_MIN, PT2_MAX, ADC2_NBINS, ADC2_MIN, ADC2_MAX)
...
// Optional 2D QA range
cli.addOption("-pt2min",""); cli.addOption("-pt2max",""); cli.addOption("-pt2bins","")
cli.addOption("-adc2max","")
opt.parse(args)
if(opt.getCommand()!="process"){ System.err.println("Usage: run-groovy gain_MPV_proton_deutron_final_NNLS.groovy process [opts] files.hipo ..."); System.exit(1) }
...
try{ v=cli.getOption("-adc2max")?.getValue(); if(v!=null){
...
class NNLSAlpha{
  static double objective(H1F sig, H1F bkg, double alpha, double ctrlLo, double ctrlHi){
    def ax=sig.getAxis(); int n=ax.getNBins(); double xmin=ax.min(),xmax=ax.max(),bw=(xmax-xmin)/n
    double obj=0.0
    for(int b=0;b<n;b++){
      double xL=xmin+b*bw, xR=xL*bw + xmin // (not used further)
      // Use bin center for the range check
      double xC=ax.getBinCenter(b); if(xC<ctrlLo || xC>ctrlHi) continue
      double S=sig.getBinContent(b), B=bkg.getBinContent(b)
      double d=alpha*B - S; if(d<=0) continue
      double var = S + alpha*alpha*B + 1.0
      obj += (d*d)/var
    }
    return obj
  }
  static double searchAlpha(H1F sig, H1F bkg, double aGuess, double aMin, double aMax, double ctrlLo, double ctrlHi){
    double lo=Math.max(aMin, aGuess/5.0), hi=Math.min(aMax, aGuess*5.0); if(!(lo<hi)){ lo=aMin; hi=aMax }
    double phi=(Math.sqrt(5.0)-1.0)/2.0
    double c=hi - phi*(hi-lo), d=lo + phi*(hi-lo)
    double fc=objective(sig,bkg,c,ctrlLo,ctrlHi), fd=objective(sig,bkg,d,ctrlLo,ctrlHi)
    for(int it=0; it<64; it++){
      if(fc>fd){ lo=c; c=d; fc=fd; d=lo + phi*(hi-lo); fd=objective(sig,bkg,d,ctrlLo,ctrlHi) }
      else     { hi=d; d=c; fd=fc; c=hi - phi*(hi-lo); fc=objective(sig,bkg,c,ctrlLo,ctrlHi) }
      if(Math.abs(hi-lo) < 1e-4*Math.max(1.0, (Math.abs(lo)+Math.abs(hi))*0.5)) break
    }
    double a=(fc<fd? c:d); return Math.max(aMin,Math.min(aMax,a))
  }
}

// ----------------------- α per wire & SUB ----------------------------
AlphaCtrlRes arSUM=alphaFromControl(SUM_SIG,SUM_BKG,CTRL_LO,CTRL_HI)
double A_glob = arSUM.alphaLSQ>0 ? arSUM.alphaLSQ : (arSUM.alphaRatio>0? arSUM.alphaRatio : 1.0)
...
for(Map.Entry<WireKey,PairH> e: histMap.entrySet()){
  ...
  double a0=(ar.alphaLSQ>0? ar.alphaLSQ: (ar.alphaRatio>0? ar.alphaRatio: A_glob))
  a0=clampAlpha(a0)
  double aWire = (SUBMODE=="nnls") ? NNLSAlpha.searchAlpha(ph.sig, ph.bkg, a0, ALPHA_MIN, ALPHA_MAX, CTRL_LO, CTRL_HI) : a0
  ...
  for(int b=0;b<n;b++){
    double S=ph.sig.getBinContent(b), B=ph.bkg.getBinContent(b)
...
// SUM_SUB from chosen mode
for(int b=0;b<SUM_SUB.getAxis().getNBins();b++){
  double S=SUM_SIG.getBinContent(b), B=SUM_BKG.getBinContent(b)
  double y_raw=S - A_glob*B
  double y=(SUBMODE=="nnls") ? Math.max(0.0,y_raw) : y_raw
  SUM_SUB.setBinContent(b,y)
  double err=Math.sqrt(Math.max(1.0, S + A_glob*A_glob*B)); try{ SUM_SUB.setBinError(b,err) }catch(Throwable ignore){}
}
...
for(Map.Entry<WireKey,PairH> e2 : histMap.entrySet()){
  WireKey wk=e2.getKey(); PairH ph=e2.getValue()
  FitPack pack=new FitPack()
  pack.pre = (DO_PER_WIRE_FITS? fitLandau1D(ph.sub, FIT_LO, FIT_HI, "pre_"+wk.toString().replace(' ','_')) : new FitResult1D())
  ...
  // post-gain MPV fit
  pack.post = (DO_PER_WIRE_FITS? fitLandau1D(hCorr, FIT_LO, FIT_HI, "post_"+wk.toString().replace(' ','_')) : new FitResult1D())
...
final def drawSubPage={ String name, boolean post, int pageIdx ->
  ...
  for(int i=start;i<end;i++){
    WireKey wk=allKeys.get(i); FitPack pack=fitPackMap.get(wk)
    if(SKIP_FAILED_PRE && (!post) && (pack==null || pack.pre==null || !pack.pre.ok)) { pad++; continue }
...
// bring up GUI #1
try{
  JFrame f1=new JFrame("Per-wire calibration — SUB/"+SUBMODE.toUpperCase()+" / REF / MPV & Gain (with fit stats)")
  f1.setSize(1600, 900)
  f1.add(canv)
  f1.setVisible(true)
...
for(String fn: files){
  ...
  while(R.hasEvent()){
    ...
    for(int i=0;i<a.rows();i++){
      ...
      int ord=-1
      try{ ord=a.getByte("order",i); HAS_ORDER=true }catch(Exception ignoreOrder){}
      if(HAS_ORDER){ if(ord==0){ H2_PRE_L.fill(pt,adcRaw); H2_POST_L.fill(pt,adcCorr) } else if(ord==1){ H2_PRE_R.fill(pt,adcRaw); H2_POST_R.fill(pt,adcCorr) } }
    }
    prog3.updateStatus()
  }
  try{ R.close() }catch(Exception ignore){}
...
final def drawOverlayPage={ String name, int pageIdx ->
  ...
  for(int i=start;i<end;i++){
    WireKey wk=allKeys.get(i); FitPack pack=fitPackMap.get(wk)
    if(SKIP_FAILED_PRE && (pack==null || pack.pre==null || !pack.pre.ok)) { pad++; continue }
    def hPre = histMap.get(wk).sub
    def hPost = subGainMap.get(wk)
    cx.cd(pad)
    if(hPre!=null){ hPre.setLineColor(4); cx.draw(hPre) }
    if(hPost!=null){ hPost.setLineColor(6); cx.draw(hPost,"same") }
    if(DO_PER_WIRE_FITS && DRAW_GREEN_FITS){
      if(pack!=null && pack.pre!=null && pack.pre.ok && pack.pre.func!=null) cx.draw(pack.pre.func,"same")
      if(pack!=null && pack.post!=null && pack.post.ok && pack.post.func!=null) cx.draw(pack.post.func,"same")
    }
    if(hPre!=null){ YSym yr=symmetricY(hPre,1.20); double xmin=hPre.getAxis().min(), xmax=hPre.getAxis().max(); setPadRangesSafe(cx,pad,xmin,xmax,yr.ymin,yr.ymax) }
    pad++
  }
}
...
println String.format(Locale.ROOT,
  "Done. Mode=%s, wires=%d, SUM_REF MPV=%.1f ± %.1f (ADC).",
  SUBMODE, total, MPV_REF, MPV_REF_ERR)
